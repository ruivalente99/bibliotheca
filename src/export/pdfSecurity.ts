...
export interface PdfPermissionsConfig {
...
function padPassword(pwd: string): Uint8Array {
  ...
  if (pwdBytes.length >= 32) {
    ...
    padded.set(pwdBytes);
    padded.set(PDF_PASSWORD_PADDING.subarray(0, 32 - pwdBytes.length), pwdBytes.length);
  }
  return padded;
}
 ...
 */
export function computePdfPermissions(opts?: PdfPermissionsConfig): number {
  ...
  const allowAnnotating = opts?.allowAnnotating === true;
  ...
  if (allowCopying) {
    ...
    p &= ~(1 << 4);
    p |= (1 << 9); // Keep accessibility extraction on for WCAG / screen readers
  }
...
export function computeUserValue(encKey: Uint8Array, fileId: Uint8Array): Uint8Array {
  const padHash = md5(PDF_PASSWORD_PADDING);
  ...
  uValue.set(result, 0);
  uValue.set(PDF_PASSWORD_PADDING.subarray(0, 16), 16);
  return uValue;
}
 ...
 */
export function encryptPdfBytes(pdfBytes: Uint8Array, options: PdfEncryptionOptions): Uint8Array {
  ...
  const permissions = computePdfPermissions(options.permissions);
  ...
  if (idMatch && idMatch[1]) {
    ...
    fileId.set(md5(toUtf8(`BIBLIOTHECA_${Date.now()}_${Math.random()}`)));
  }
