...
export interface NanoBananaLogoProps {
  ...
  /** Accessible label */
  ariaLabel?: string;
  /** Optional custom emblem content to replace the default vector sticker */
  ...
  return (
    <div
      ...
      className={cn(
        ...
        glow
          ? "shadow-sm shadow-[var(--brand-ring)]"
          : "shadow-2xs",
        className,
        classNames.root
      )}
    >
      {children ? (
        <span className="pointer-events-none select-none shrink-0 flex items-center justify-center">
          {children}
        </span>
      ) : (
        <svg
