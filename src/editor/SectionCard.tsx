...
export interface SectionCardProps {
  ...
  /** Optional counter or badge text */
  badge?: string | number;
...
export function SectionCard({
  ...
  return (
    ...
    >
      {/* Card Header */}
      <div className="flex items-center justify-between p-4 border-b border-stone-100 dark:border-[#21262d]">
        <div className="flex items-center gap-2.5 min-w-0">
          {icon && <span className="text-amber-600 dark:text-amber-400 shrink-0">{icon}</span>}
          <h2 className="text-xs sm:text-sm font-bold tracking-wider uppercase text-stone-800 dark:text-[#f0f3f6] truncate">
            {title}
          </h2>
          {badge !== undefined && (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-stone-100 dark:bg-[#21262d] text-stone-600 dark:text-[#8b949e]">
              {badge}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {collapsible && (
            <button
              ...
              className="p-1 rounded-lg hover:bg-stone-100 dark:hover:bg-[#21262d] text-stone-400 dark:text-[#8b949e] transition-colors cursor-pointer"
            >
